...
export const getSingleCategory = createAsyncThunk("category/fetch-single", async (categoryId, thunkAPI) => {
  try {
    return await categoryService.getSingleCategory(categoryId);
  } catch (error) {
    const message = (error.response && error.response.data && error.response.data.message) || error.message || error.toString();
    return thunkAPI.rejectWithValue(message);
  }
